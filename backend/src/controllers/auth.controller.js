...
export const me = catchAsync(async (req, res) => {
  ...
  return ApiResponse.send(res, { data: { user } });
});

export const updateProfile = catchAsync(async (req, res) => {
  const user = await authService.updateProfile(req.user.id, req.body);
  return ApiResponse.send(res, { message: MESSAGES.AUTH.PROFILE_UPDATED, data: { user } });
});
...
export const uploadAvatar = catchAsync(async (req, res) => {
  ...
  return ApiResponse.send(res, { message: MESSAGES.AUTH.AVATAR_UPLOADED, data: { user } });
});
